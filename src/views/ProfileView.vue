  ...
  <div class="content">
    <div class="row d-flex main-content">
      ...
      <div class="tt col-11 col-lg-7">
        ...
        <!---Ten Tai Khoan-->
        <h4 class="name text-center">{{ store.user.full_name }}</h4>
        ...
        <div class="tt row">
          <div class="muc display-flex flex-direction-column">
            <ul class="d-flex justify-content-between hang so1">
              ...
              <li class="col-4 text-center">{{ store.user.username }}</li>
            ...
            <ul class="d-flex justify-content-between hang so2">
              ...
              <li class="col-4 text-center">{{ store.user.full_name }}</li>
              <li class="col-3 text-right">
            ...
            <ul class="d-flex justify-content-between hang so3">
              ...
              <li class="col-4 text-center">{{ store.user.money.toLocaleString() }}đ</li>
              <li class="col-3 text-right">
            ...
            <ul class="d-flex justify-content-between hang so4">
              ...
              <li class="col-4 text-center">{{ store.user.email }}</li>
              <li class="col-3 text-right">
                <button><i class="bi bi-pencil-square"></i>Sửa</button>
              </li>
            </ul>
            <ul class="d-flex justify-content-between hang so5">
              ...
              <li class="col-4 text-center" :style="'color: ' + (store.user.status === 0 ? 'green' : 'red')">
                {{ store.user.status === 0 ? 'Đang hoạt động' : 'Tạm khóa' }}</li>
              <li class="col-3 text-right">
                <button><i class="bi bi-eye"></i>Xem</button>
              </li>
            </ul>
    ...
    <div class="row d-flex justify-content-center">
      <div class="col-lg-6 col-sm-10">
        ...
        <div class="row d-flex align-items-center justify-content-evenly" v-for="(api, index) in list_api" :key="index">
          ...
          <div class="col-7">
            <input type="text" readonly class="api_name_input"
              :value="'http://captchanro.com/captcha/solve?token=' + api" />
          </div>
          <div class="col-2">
...
export default {
  ...
  methods: {
    loadAPI() {
      axios
        .post('https://run.captchanro.com/token/load', { uid: this.$cookies.get('user').uid })
        .then((res) => {
    ...
    createAPI() {
      axios.post('https://run.captchanro.com/token/add', { uid: this.$cookies.get('user').uid }).then(() => {
        this.loadAPI()
      })
    },
    deleteAPI(token) {
      axios.post('https://run.captchanro.com/token/delete', { uid: this.$cookies.get('user').uid, token: token }).then(() => {
        this.loadAPI()
      })
    },
    logout() {
